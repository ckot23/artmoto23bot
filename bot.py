...
def main_menu(is_admin: bool) -> ReplyKeyboardMarkup:
    """Короткое меню: все частые действия доступны без команд."""
    rows: list[list[KeyboardButton]] = []
    if WEBAPP_URL.startswith("https://"):
        rows.append([KeyboardButton(text="🎨 Рассчитать и заказать", web_app=WebAppInfo(url=WEBAPP_URL))])
    rows.append([KeyboardButton(text="📦 Мои заказы"), KeyboardButton(text="❓ Как заказать")])
    rows.append([KeyboardButton(text="ℹ️ О плёнке"), KeyboardButton(text="☎️ Контакты")])
    if is_admin:
...
def admin_panel() -> InlineKeyboardMarkup:
    ...
    b.button(text="🤝 Подтверждённые", callback_data="adm:list:confirmed")
    b.button(text="🖨 В печати", callback_data="adm:list:production")
    b.button(text="✅ Завершённые", callback_data="adm:list:done")
...
def user_order_buttons(rows: list[dict[str, Any]]) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    for row in rows:
        b.button(
            text="#" + str(row["id"]) + " · " + str(row["total"]) + " ₽",
            callback_data="usr:order:" + str(row["id"]),
        )
    b.adjust(1)
    return b.as_markup()
...
WELCOME = (
    "<b>Наклейки на заказ</b> 👋\n\n"
    "Рассчитайте цену и оформите заказ за пару минут:\n"
    "1. Выберите плёнку и цвет\n"
    "2. Укажите размер и тираж\n"
    "3. Оставьте контакт\n\n"
    "💧 Не боятся воды · ☀️ до 7 лет · 📏 от 1 см до 5 м\n\n"
    "Нажмите кнопку конструктора ниже."
)
...
async def cmd_start(message: Message) -> None:
    ...
    if WEBAPP_URL.startswith("https://"):
        await message.answer("Кнопка конструктора уже внизу 👇")
...
async def cmd_app(message: Message) -> None:
    if not WEBAPP_URL.startswith("https://"):
        ...
        return
    await message.answer(
        "Нажмите кнопку конструктора внизу 👇",
        reply_markup=main_menu(message.from_user.id in ADMIN_IDS),
    )


@client.message(Command("help"))
@client.message(F.text == "❓ Как заказать")
async def cmd_help(message: Message) -> None:
    await message.answer(
        "<b>Как оформить заказ</b>\n\n"
        "1. Откройте конструктор кнопкой «🎨 Рассчитать и заказать».\n"
        "2. Выберите параметры — цена пересчитается автоматически.\n"
        "3. Укажите контакт и нажмите «Оформить».\n"
        "4. Если есть макет, отправьте его следующим сообщением.\n\n"
        "Команды: /start · /app · /myorders · /help",
        reply_markup=main_menu(message.from_user.id in ADMIN_IDS),
    )


@client.message(F.text == "☎️ Контакты")
async def contacts(message: Message) -> None:
    await message.answer(
        "Оставьте телефон, email или @username при оформлении — менеджер свяжется с вами. "
        "Готовый макет можно отправить сюда файлом сразу после заказа."
    )
...
async def my_orders(message: Message) -> None:
    ...
    await message.answer(
        ...
        reply_markup=user_order_buttons(rows),
    )


@client.callback_query(F.data.startswith("usr:order:"))
async def user_order_detail(callback: CallbackQuery) -> None:
    raw_id = callback.data.rsplit(":", 1)[-1]
    row = get_order(int(raw_id)) if raw_id.isdigit() else None
    if not row or int(row["user_id"]) != callback.from_user.id:
...
async def on_layout_file(message: Message) -> None:
    latest = list_orders(user_id=message.from_user.id, limit=1)
    order_hint = " к заказу #" + str(latest[0]["id"]) if latest else ""
    delivered = False
    for admin_id in ADMIN_IDS:
        try:
            ...
            await message.bot.send_message(
                admin_id,
                "📎 Макет" + order_hint + " от " + html_lib.escape(message.from_user.full_name or "клиента")
                + " (<code>" + str(message.from_user.id) + "</code>)",
            )
            delivered = True
        except Exception as error:  # noqa: BLE001
            log.warning("Не удалось переслать файл %s: %s", admin_id, error)
    await message.answer(
        "Файл получен" + order_hint + (" и передан менеджеру 👌" if delivered else ". Менеджер увидит его после настройки уведомлений.")
    )
...
async def run() -> None:
    ...
    await bot.set_my_commands(
        [
            ...
            BotCommand(command="app", description="Рассчитать и заказать"),
